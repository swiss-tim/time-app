...
@st.cache_data
def load_csv(path):
    # Load the CSV. Now includes activityName for grouping.
    return pd.read_csv(
        path,
        usecols=['activityCategoryName', 'activityName', 'activityStartDate', 'activityDuration [ms]'],
        sep=',',
        engine='python'
    )
...
# build once (cached) near top so clicks are fast
...
def timeline_for_period(period):
    ...
    with col2:
        st.write(f"Selected {period}: {st.session_state.get(f'current_{period}')}")
    ...
    for cat in ordered_cats:
        ...
        if has_activity:
            for act in sorted(cat_df['activityName'].unique()):
                act_hours = cat_df[cat_df['activityName'] == act]['duration_hours'].sum()
